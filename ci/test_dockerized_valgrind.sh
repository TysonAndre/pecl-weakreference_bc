...
PHP_CONFIGURE_ARGS="--disable-all --enable-zts --enable-debug --with-valgrind"
if [[ "$ARCHITECTURE" == i386 ]]; then
	PHP_IMAGE="$ARCHITECTURE/php"
	DOCKER_IMAGE_VALGRIND="weakreference_bc-valgrind-test-runner:$ARCHITECTURE-$PHP_VERSION_FULL"
	CFLAGS="$CFLAGS -m32"
else
	PHP_IMAGE="php"
	DOCKER_IMAGE_VALGRIND="weakreference_bc-valgrind-test-runner:$ARCHITECTURE-$PHP_VERSION"
fi

DOCKER_IMAGE_VALGRIND=weakreference_bc-valgrind-test-runner:$PHP_VERSION_FULL
docker build \
